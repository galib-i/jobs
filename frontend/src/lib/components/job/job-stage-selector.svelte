...
{#if job.isActive}
  <div class="ml-auto inline-block {isAddingStage ? 'z-40' : 'mt-0.5'}">
    ...
    {#if isAddingStage}
      ...
      >
        <div
          class="mb-2 border-b border-slate-200 px-3 pb-2 text-center text-xs font-bold tracking-wider text-slate-500 uppercase dark:border-slate-700 dark:text-slate-400"
        >
          Select Stage
        </div>
        <div
          class="custom-scrollbar max-h-48 space-y-0.5 overflow-y-auto overscroll-contain px-1.5 pb-1.5"
        >
          {#each stagesToSelect as stage}
            ...
            <button
              disabled={isDisabled}
              class="block w-full rounded px-2.5 py-1.5 text-center text-sm font-bold transition-colors {isDisabled
              ...
              style={isHovered && !isDisabled
                ? `background-color: ${stageBg}; color: ${stageText}`
                : ""}
